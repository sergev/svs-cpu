...
#
# Print instruction with mnemonics.
#
def print_cmd(reg, opcode, addr):
    reg = int(reg, 8)
    addr = int(addr, 8)
    ...
    if len(opcode) == 2:
        # Long address format
        opcode = int(opcode, 8)
        result = opname_long_bemsh[opcode & 0o17]
    else:
        # Short address format
        if opcode[0] != '0':
            addr |= 0o70000
        opcode = int(opcode[1:], 8)
        result = opname_short_bemsh[opcode]

    if addr >= 0o77700:
        result += " -%o" % ((addr ^ 0o77777) + 1)
    elif addr > 0:
        result += " %o" % addr
    ...
    if n > 0:
        ...
        if cmd == "к":
            ...
            if instruction_space(addr):
                ...
                oct_file.write("i %05o %s, %s\n" % (addr,
                    print_cmd(word[1], word[2], word[3][:-1]),
                    print_cmd(word[4], word[5], word[6])))
            else:
